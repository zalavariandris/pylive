...
class Window(QWidget):
    ...
    def __init__(self, parent:QWidget|None=None):
        ...
        self.graph_view.setModel(self.nodes, self.edges)
    ...
    def deserialize(self, text:str)->bool:
        ...
        ### create node items
        _node_row_by_id = dict() # keep node name as references for the edge relations
        ...
        for row, node in enumerate(data['nodes']):
            ...
            _node_row_by_id[node['id']] = row

        print(_node_row_by_id)
        ...
        #     source_row = _node_row_by_id[source_node_id]
        #     target_row = _node_row_by_id[target_node_id]
    ...
    def _evaluate_node(self, node_index:QModelIndex|QPersistentModelIndex):
        ...
        kwargs = dict()
        inputs = [_ for _ in self.edges.in_edges(node_index)]
        print("in edges:", inputs)
        for edge_item in self.edges.in_edges(node_index):
            print(f"EVALUATE SOURCE {edge_item.inlet}: {edge_item.source}")
            kwargs[edge_item.inlet] = self._evaluate_node(edge_item.source)
