...
class LiveScript(QWidget):
    def __init__(self, parent=None):
        ...
        self.watcher.fileChanged.connect(
            lambda: self.on_file_change(self.filepath)
        )
    ...
    def config(self):
        config = {"open_recent_file": False, "recent": [], "live": True}
    ...
    def evaluate(self):
        ...
        try:
            ...
            the_ast = ast.parse(
                self._sourcecode, filename="<script>", type_comments=True
            )
            try:
                ...
                self.display_exception(
                    e, prefix="Syntax error while compiling:\n"
                )
    ...
    def setupMenuBar(self):
        ...
        zoom_in_action.triggered.connect(lambda: self.increaseFontSize())
        zoom_out_action = QAction("Zoom Out", self)
        zoom_out_action.triggered.connect(lambda: self.decreaseFontSize())
        ...
        insert_template_action.triggered.connect(
            lambda: self.script_edit.insertPlainText(TEMPLATE_SCRIPT)
        )
    ...
    def saveFile(self, filepath: str | None = None):
        assert filepath is None or isinstance(
            filepath, str
        ), f"got:, {filepath}"
        ...
        if not self.filepath or filepath:
            choosen_filepath, filter_used = QFileDialog.getSaveFileName(
                self, "Save", ".py", "Python Script (*.py);;Any File (*)"
            )
    ...
    def openFile(self, filepath: str | None = None):
        ...
        if not filepath:
            ...
            choosen_filepath, filter_used = QFileDialog.getOpenFileName(
                self, "Open", ".py", "Python Script (*.py);;Any File (*)"
            )
            filepath = choosen_filepath
