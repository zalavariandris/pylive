...
def format_type(annotation):
    ...
    if hasattr(annotation, "__name__"):  # For built-in types like int, float
        ...
        args = (
            ", ".join(format_type(arg) for arg in annotation.__args__)
...
class ParamInlet(Pin):
    ...
    def color(self) -> str:
        if (
            self._param.default is inspect.Parameter.empty
            and self._param.kind != inspect.Parameter.VAR_POSITIONAL
            and self._param.kind != inspect.Parameter.VAR_KEYWORD
        ):
            return "red"
...
class NodeWidget(QGraphicsWidget):
    def __init__(self, parent: QGraphicsItem | None = None):
        ...
        self._pen: QPen = QPen(
            QBrush(palette.color(QPalette.ColorRole.Text)), 1
        )
...
class FunctionNodeWidget(NodeWidget):
    ...
    def paint(
        ...
        painter.drawText(
            int(rect.x() + x), int(rect.y() + fm.ascent() - 1), "("
        )
        painter.drawText(
            int(rect.right()), int(rect.y() + fm.ascent() - 1), ")"
        )
...
class ExpressionWidget(NodeWidget):
    ...
    def setExpression(self, expression: str):
        ...
        for name, col_offset, end_col_offset in unbound_nodes:
            ...
            expression_parts.append(("text", expression[pos:col_offset]))

            expression_parts.append(
                ("var", expression[col_offset:end_col_offset])
            )
            pos = end_col_offset
        expression_parts.append(("text", expression[pos:]))
