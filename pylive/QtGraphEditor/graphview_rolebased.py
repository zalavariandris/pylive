...
class NodeItem(QGraphicsItem):
		...
		if (self.parent_graph.graph_model and
			...
			match change:
				case QGraphicsItem.GraphicsItemChange.ItemPositionHasChanged:
					...
					node_index = NodeIndex(self.persistent_node_index)
					
					new_pos = self.pos()
					posx = int(new_pos.x())
					posy = int(new_pos.y())

					graph.setNodeData(node_index, (posx, posy), NodeDataRole.LocationRole)
	...
	def onNameContentsChanged(self):
		if (self.parent_graph.graph_model and
			...
			graph.setNodeData(node_index, new_name, NodeDataRole.NameRole)
		...
		if self.persistent_edge_index and self.persistent_edge_index.isValid():
			if QGraphicsItem.GraphicsItemChange.ItemSelectedHasChanged:
				if self.parent_graph.graph_model and self.parent_graph.edges_selectionmodel:
					...
					graph:GraphModel = self.parent_graph.graph_model
					edge_index = self.persistent_edge_index
...
class GraphView(PanAndZoomGraphicsView):
	...
	def setModel(self, graph_model:GraphModel):
		...
		self.handleNodesInserted(  NodeIndex(), 0, self.graph_model.nodeList.rowCount()-1)
		self.handleInletsInserted( InletIndex(), 0, self.graph_model.inletList.rowCount()-1)
		self.handleOutletsInserted(OutletIndex(), 0, self.graph_model.outletList.rowCount()-1)
		self.handleEdgesInserted(  EdgeIndex(), 0, self.graph_model.edgeList.rowCount()-1)

		self.graph_model.nodeList.rowsInserted.connect(self.handleNodesInserted)
		self.graph_model.nodeList.dataChanged.connect(self.handleNodesDataChanged)
		self.graph_model.nodeList.rowsAboutToBeRemoved.connect(self.handleNodesRemoved)

		self.graph_model.inletList.rowsInserted.connect(self.handleInletsInserted)
		self.graph_model.inletList.dataChanged.connect(self.handleInletsDataChanged)
		self.graph_model.inletList.rowsAboutToBeRemoved.connect(self.handleInletsRemoved)

		self.graph_model.outletList.rowsInserted.connect(self.handleOutletsInserted)
		self.graph_model.outletList.dataChanged.connect(self.handleOutletsDataChanged)
		self.graph_model.outletList.rowsAboutToBeRemoved.connect(self.handleOutletsRemoved)

		self.graph_model.edgeList.rowsInserted.connect(self.handleEdgesInserted)
		self.graph_model.edgeList.dataChanged.connect(self.handleEdgesDataChanged)
		self.graph_model.edgeList.rowsAboutToBeRemoved.connect(self.handleEdgesRemoved)
	...
	def handleNodesInserted(self, parent:NodeIndex, first:int, last:int):
		...
		if self.graph_model:
			for row in range(first, last+1):
				...
				# update gaphics item
				self.handleNodesDataChanged(node, node)
	...
	def handleNodesRemoved(self, parent:NodeIndex, first:int, last:int):
		...
		if self.graph_model:
			for row in range(last, first-1, -1):
				node = NodeIndex(self.graph_model.nodeList.index(row, 0))
				node_item = self.index_to_item_map[node]
				node_item.destroy()
	...
	def handleNodesDataChanged(self, topLeft:NodeIndex, bottomRight:NodeIndex, roles=[]):
		if self.graph_model:
			for row in range(topLeft.row(), bottomRight.row()+1):
				...
				node_index = NodeIndex(graph.nodeList.index(row, 0))
				...
				for col in range(topLeft.column(), bottomRight.column()+1):
					node = NodeIndex(self.graph_model.nodeList.index(row, col))
					match col:
						case 0:
							pass
						case 1:
							new_name = str(self.graph_model.getNodeData(node, NodeDataRole.NameRole))
