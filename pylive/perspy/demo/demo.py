...
class PerspyApp():
            ...
            if error_msg is not None:
                ...
                if self.solver_results is None:
                    ...
                    if ui.begin_attribute_editor("res props"):
                        ...
                        imgui.set_next_item_width(260)
                        imgui.input_text("##euler", f"{math.degrees(euler.x):.2f}, {math.degrees(euler.y):.2f}, {math.degrees(euler.z):.2f}" if self.solver_results.orientation is not None else "N/A", flags=imgui.InputTextFlags_.read_only)
    ...
    def update_solve(self):
        ...
        # scale scene
        match self.doc.reference_distance_mode:
