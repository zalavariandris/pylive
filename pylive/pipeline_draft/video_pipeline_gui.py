...
# class Node:
#     def __init__(self, factory: Callable[..., Any], inputs: Dict[Self]={}, parameters: Dict[str, Any]={}):
#         self.factory = factory
#         self.inputs = inputs
#         self.parameters = parameters

#     def set_input(self, name: str, value: Node) -> None:
...
#         baked_inputs = {name: input_node.evaluate() for name, input_node in self.inputs.items()}
#         return self.factory(**baked_inputs, **self.parameters)

#     # def bake(self, baked_inputs: Dict[str, Any]) -> Any:
#     #     return self.factory(**baked_inputs, **self.parameters)
...
# def evaluate(root: Node) -> Any:
#     sorted_nodes = topological_sort(root)
#     results = {}
#     for node in sorted_nodes:
#         inputs = {name: results[input_node] for name, input_node in node.inputs.items()}
#         instance = node.factory(**baked_inputs, **node.parameters)
...
def bake(engine_graph: nx.MultiDiGraph, target_node: str) -> Callable | None:
    ...
    for node_id in nx.topological_sort(subgraph):
        node_data:Node = subgraph.nodes[node_id]
        ...
        # 5. Bake the closure
        try:
            instance = node_data.bake(inputs)
...
def diff(prev: nx.MultiDiGraph, new: nx.MultiDiGraph) -> Dict[str, List[Any]]:
    ...
    diffs["added_nodes"] = list(new_nodes - prev_nodes)
    diffs["removed_nodes"] = list(prev_nodes - new_nodes)
    
    prev_edges = set(prev.edges(keys=True))
    new_edges = set(new.edges(keys=True))
    
    diffs["added_edges"] = list(new_edges - prev_edges)
    diffs["removed_edges"] = list(prev_edges - new_edges)
    
    # Check for parameter changes in common nodes
    common_nodes = prev_nodes & new_nodes
    for node in common_nodes:
        prev_params = prev.nodes[node].get('parameters', {})
        new_params = new.nodes[node].get('parameters', {})
        
        if prev_params != new_params:
            diffs["parameter_changes"].append((node, prev_params, new_params))
    
    # Check for input changes (edge attribute changes)
    # For edges that exist in both, check if their attributes changed
    common_edges = prev_edges & new_edges
    for edge in common_edges:
        src, dst, key = edge
        prev_data = prev.get_edge_data(src, dst, key=key)
        new_data = new.get_edge_data(src, dst, key=key)
        
        if prev_data != new_data:
            diffs["input_changes"].append((edge, prev_data, new_data))
    
    return diffs

def patch(engine_graph: nx.MultiDiGraph, diffs: Dict[str, List[Any]]) -> None:
    """Apply the computed differences to the engine graph."""
    # Remove nodes first (to avoid orphaned edges)
    for node in diffs["removed_nodes"]:
        engine_graph.remove_node(node)
    
    # Remove edges
    for edge in diffs["removed_edges"]:
        src, dst, key = edge
        engine_graph.remove_edge(src, dst, key=key)
    
    # Add new nodes
    for node in diffs["added_nodes"]:
        engine_graph.add_node(node)
    
    # Add new edges
    for edge in diffs["added_edges"]:
        src, dst, key = edge
        engine_graph.add_edge(src, dst, key=key)
    
    # Apply parameter changes
    for node, prev_params, new_params in diffs["parameter_changes"]:
        if 'parameters' not in engine_graph.nodes[node]:
            engine_graph.nodes[node]['parameters'] = {}
        engine_graph.nodes[node]['parameters'].update(new_params)
    
    # Apply input changes (edge attribute changes)
    for (src, dst, key), prev_data, new_data in diffs["input_changes"]:
        engine_graph[src][dst][key].update(new_data)
