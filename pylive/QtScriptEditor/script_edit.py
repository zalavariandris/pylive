...
class ScriptEdit(QPlainTextEdit):
    ...
    # def sizeHint(self):
    #     """Returns the suggested size based on the document height."""
    #     metrics = QFontMetrics(self.font())
    #     line_count = self.blockCount()  # Get the number of lines
    #     document_height = (line_count+1) * metrics.lineSpacing()  # Add some padding

    #     # Get the vertical scrollbar height
    #     scrollbar_visible = self.verticalScrollBar().isVisible()
    #     scrollbar_height = 0
    #     if scrollbar_visible:
    #         scrollbar_height = self.verticalScrollBar().sizeHint().height()
    #     print(document_height+scrollbar_height)

    #     return QSize(super().sizeHint().width(), document_height+scrollbar_height)
